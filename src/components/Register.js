  ...
  return(
    ...
    validate={(valores)=>{
      ...
      if(!valores.password){
        ...
        errores.password='La contraseña puede contener de 4 a 12 caracteres';
      }

      if(!valores.repeat_pass){
        errores.repeat_pass='Por favor capture la confirmacion de la contraseña'
        // console.log('password');
    ...
    >
      ...
      <form className='formReg' onSubmit={handleSubmit}>
         ...
         <input
          ...
          onBlur={handleBlur}
        />
          {touched.repeat_pass && errors.repeat_pass && <div className='error'>{errors.repeat_pass}</div>}
